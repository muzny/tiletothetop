...
function dropTileInEmptyTile(a){a.preventDefault();if(0==$("#"+a.currentTarget.id).children().length){var b=a.dataTransfer.getData("Text"),b=document.getElementById(b);a.currentTarget.appendChild(b);$(b).centerOnParent();$(b).removeClass("inTileArea");checkGameWon()}};var board=null,messenger=null,timer=null,score=0,numberHintsUsed=0,HINT_PENALTY=100,MAX_FONT=36,TILE_SIZE=60,TIMER_PENALTY=10,TIMER_CUTOFF=60,ANSWER_CHEAT_ON=-1!=window.location.host.indexOf("localhost")||-1!=window.location.host.indexOf("127.0.0.1"),MODAL_IDS=["#login-modal","#register-modal"],MAX_DIFFICULTY=150,INCR_DIFFICULTY=5,isPrevStepLess=!1,expDifficultyStep=INCR_DIFFICULTY,NUM_WORDS=4,MAX_WORDLEN=10,inStartMenu=!1,setupEvents=!1,gameIsStarted=!1;
$(window).load(function(){$.ajaxSetup({cache:!1});initializeMenuButtons();initializeWordListButtons();messenger=new Messenger;getAccountData();getUserRank();getLeaderboard(10);$("#facebook-submit").click(sendFacebookPost);$("#facebook-cancel").click(removeFacebookModal);createStaticGameIfApplicable()});function initializeBoard(a){null!=board&&($("#definitions-answers-area").remove(),$("#tiles-area").remove());board=new Board(a);$(".jtextfill").textfill({maxFontPixels:MAX_FONT})}
...
function isPaused(){for(var a=0;a<MODAL_IDS.length;a++)if($(MODAL_IDS[a]).hasClass("in"))return!0;return!$("#play").hasClass("active")}function scoreFunc(a){return 100*a.length+0}function generateShareUrl(){button=$("#share");linkArea=$("#linkArea");facebook=$("#post-to-facebook");button.addClass("hidden-button");button.removeClass("shown-button");linkArea.addClass("shown-button");linkArea.removeClass("hidden-button");facebook.addClass("shown-button");facebook.removeClass("hidden-button")}
...
!1;expDifficultyStep<MAX_DIFFICULTY&&(expDifficultyStep*=2);incrementDifficulty(expDifficultyStep)})}function incrementDifficulty(a){var b=parseInt($("#setting-difficulty").val()),b=Math.max(0,Math.min(MAX_DIFFICULTY,b+a));$("#setting-difficulty").val(b);a=50>b?"Easy":50<=b&&100>b?"Medium":"Hard";$("#difficulty-text").text(a)}function returnToGame(){$("#play.carousel").carousel("next");$("#start-menu").hide();inStartMenu=!1}
function returnToStart(){null!=board&&$("#start-game-inprogress").show();$("#start-menu").show();$("#play.carousel").carousel("prev");inStartMenu=!0}
function startGame(){gameIsStarted&&resetEvents();gameIsStarted=!0;$("#drag-blocker").css({display:"hidden","z-index":"-1"});var a=$("#game-options .accordion-body.in");if(0==a.size())messenger.getWords(initializeBoard,null,null);else if("dict-game"==a[0].id){var a=parseInt($("#setting-difficulty").val()),b=parseInt($("#setting-tag").val());isNaN(b)&&(b=null);messenger.getWords(initializeBoard,a,b)}else custom_list=$("#setting-custom").val(),messenger.getCustomWords(initializeBoard,custom_list);showGameButtons()}
function quitGame(){for(var a=window.board.definitions.getDefinitions(),b=0;4>b;b++)for(var c=0;c<a[b].length;c++)window.board.definitions.displayHint(b);$("#score-breakdown").hide();TransitionScreen(!1,0)}function restart(){resetEvents();startGame()}function resetEvents(){$(document).off("keydown");$(document).off("shown");$(document).off("hidden");$(document).off("keypress")}
...
$("#post-to-facebook").addClass("hidden-button")}function showGameOverButtons(){$("#new-game").removeClass("hidden-button");$("#new-game").addClass("shown-button");$("#quit-game").removeClass("shown-button");$("#quit-game").addClass("hidden-button");$("#restart").removeClass("hidden-button");$("#restart").addClass("shown-button")}function transitionClick(){$("#transition-screen").css({display:"hidden","z-index":"-1"})}
function postToFacebook(){$("#facebook-form-container").removeClass("hide");$("#id_link").val($("#linkArea").text());$("#id_message").val("I just got a score of {score} on Tile To The Top!  Can you beat my score?")};var Messenger=function(){var a=window.location.host,b=-1!=a.indexOf("localhost")||-1!=a.indexOf("127.0.0.1");this.getWords=function(a,d,g){var e={word_count:NUM_WORDS,max_wordlen:MAX_WORDLEN};null!=d&&(e.difficulty=d);null!=g&&(e.tag_filter=g);$.ajax({url:"/random-words/",type:"GET",dataType:"json",data:e,success:function(b){a(b)},error:function(){b&&alert("getWords ajax error")}})};this.getStaticWords=function(a,d){$.ajax({url:"/static-words/",type:"GET",dataType:"json",data:{id:d},success:function(b){a(b)},
...
alert("getLeaderboard ajax error")}})};this.getUserRank=function(){$.ajax({url:"/get-user-rank/",type:"GET",dataType:"json",success:function(a){insertUserRank(a)},error:function(){b&&alert("getUserRank ajax error")}})};this.postToFacebook=function(a,b){data={message:b,link:a};$.ajax({url:"/post-to-facebook/",type:"POST",dataType:"json",data:data,headers:{"X-CSRFToken":$.cookie("csrftoken")},success:function(){},error:function(){}})}};$(function(){0<$("#register-modal").find(".help-inline").length||0<$("#register-modal").find(".alert").length?$("#register-modal").modal("show"):(0<$("#login-modal").find(".help-inline").length||0<$("#login-modal").find(".alert").length)&&$("#login-modal").modal("show");if("#_=_"==window.location.hash||"#"==window.location.hash)window.location.hash="";"#register-modal"==window.location.hash&&($("#register-modal").modal("show"),window.location.hash="");"#login-modal"==window.location.hash&&($("#login-modal").modal("show"),
window.location.hash="")});(function(a){a.fn.textfill=function(b){var b=b.maxFontPixels,c=a("span:visible:first",this),d=a(this).height(),g=a(this).width(),e,f,j=a(c[0]).text(),h=a(c[0]);a.each(c,function(b){a(c[b]).text().length>j.length&&(j=a(c[b]).text(),h=a(c[b]))});do h.css("font-size",b),e=h.height(),f=h.width(),b-=1;while((e>d||f>g)&&3<b);c.css("font-size",b);return this}})(jQuery);var TileArea=function(a){var b=$("<div>").attr("id","tiles-area");$("#game-area").append(b);var c=b.width();b.offset();var d=parseInt(c/(TILE_SIZE+10));this.shuffle(a);for(var c=Math.max(2,Math.ceil(a.length/d)),g,e=0;e<c*d;e++){var f=parseInt(e/d),j=e%d;0==j&&(g=$("<div>").addClass("tile-row"),b.append(g));var h=$("<div>");h.attr("id","tileBox"+f+"_"+j);h.addClass("tile-box");h.attr({ondrop:"dropTileInTileArea(event)",ondragover:"dragTileOverTileArea(event)",ondragleave:"leaveTileArea(event)"});
...
function deleteBoxAt(a){var b=$("#"+a),c=b.children().length,d=!1;0==c&&(b=getBoxAtOffset(a,-1),d=!0,c=b.children().length);1==c&&(a=b.children()[0],addToTileArea(a));if(d&&(d=$(".clicked")[0],a=getBoxAtOffset($(d).attr("id"),-1)))$(d).removeClass("clicked"),$(a).addClass("clicked")};function sendFacebookPost(){var a=$("#id_message").val(),b=$("#id_link").val(),c=$("#score-final").text(),a=a.replace("{score}",c);$("#facebook-form-container").addClass("hide");messenger.postToFacebook(b,a)}function removeFacebookModal(){$("#facebook-form-container").addClass("hide")};
